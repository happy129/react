...
router.post("/getMenuOne", (req, res) => {
    var menuId = req.body.params.menuId;
...
router.post('/updateComment',(req,res)=>{
    var commentInfo = req.body.params.commentInfo
...
router.post("/upload-avatar", avatarUpload, (req, res) => {
    ...
    console.log('这是上传图片的地址:'+newName);
    var mobile = req.username;
    console.log(mobile);
...
router.post('/uploadMenu',(req,res)=>{
    var menuName = req.body.params.menuName;
    var username = req.body.params.username;
    var foods = req.body.params.foods;
    var describe = req.body.params.describe;
    var menuTime = req.body.params.menuTime;
    var nowTime = new Date();
    var time = dateFormat(nowTime);
    // 输出菜谱信息
...
router.post('/getMyMenu',(req,res)=>{
    ...
    conn((err,db)=>{
        setError(err,res,db);
        db.collection('myMenu').find({username}).sort({time:-1}).toArray((err, result) => {
            setError(err, res, db);
            res.json({
                code: 200,
                msg: "获取我的菜谱成功",
